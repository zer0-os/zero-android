...
fun ChannelPager(
	...
	HorizontalPager(state = pagerState, count = groupChannelUiState.channelCategories.size) { index ->
		Column(modifier = Modifier.fillMaxSize()) {
			LazyColumn {
				groupChannelUiState.groupMessages(groupChannelUiState.channelCategories[index].name)?.let {
					items(it) { channel -> ChannelsItemsList(channel = channel, onClick = onClick) }
				}
			}
		}
	}
